...
func RegisterUploadRoutes(e *echo.Echo, uploadHandler *handlers.UploadHandler, sessionManager services.SessionManager) {
	...
	e.POST("/save-metadata", uploadHandler.HandleMetadataSave, sessionManager.RequireAuth) // <<< ADDED ROUTE

	// Action endpoint to handle the saving of edited metadata
	e.POST("/toggle-delete", uploadHandler.ToggleDelete, sessionManager.RequireAuth) // <<< ADDED ROUTE
